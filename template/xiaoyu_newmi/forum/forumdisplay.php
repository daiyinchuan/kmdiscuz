...
<!--{if $_G['cache']['plugin']['xiaoyu_xiaomflist']}-->
<!--{eval require './source/plugin/xiaoyu_xiaomflist/xiaoyus.inc.php';}-->
<!--{/if}-->
...
<div id="pt" class="bm cl">
	<div class="z">
		<a href="forum.php">{$_G[setting][navs][2][navname]}</a>$navigation <a href="forum.php?mod=viewthread&tid=$_G[tid]">$_G[forum_thread][short_subject]</a>
	</div>
</div>-->
...
<div id="ct" class="wp cl" {if $xiaoyuallow}style="margin-top:10px"{/if}>
<!--{if empty($xiaoyuallow)}-->

<div class="contain_header" style="margin-top: 100px;">
					<div class="wrap">
						<!--{if $_G[forum][banner] && !$subforumonly}--><img src="$_G[forum][banner]" alt="$_G['forum'][name]" /><!--{else}--><img src="template/xiaoyu_newmi/style/img/f_banner.jpg" alt="$_G['forum'][name]" /><!--{/if}-->
					</div>
					<div class="contain_header_con">
						<div>
							<a href="forum.php?mod=forumdisplay&fid=$_G[fid]"><h2>$_G['forum'][name]</h2></a>
						...
						<div class="xiaoyu_intro">
							<span class="intro cl">
                            <!--{if $_G['forum']['rules']}-->
                            <!--{if $moderatedby}-->{lang forum_modedby}: <span class="kym_aduser">$moderatedby</span> &nbsp;&nbsp;&nbsp;<!--{/if}--> $_G['forum'][rules]
                            <!--{else}-->
                            暂无本版块规则,请期待
                            <!--{/if}-->
                            </span>
                         ...
                         <!--{if !$_GET['archiveid']}--><a href="javascript:;" class="xiaoyu_post_btn" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} title="{lang send_posts}">+发表新主题</a><!--{/if}-->
				</div>
...
<!--{if $leftside}-->
		...
		<!--{/if}-->
		<div class="kym_mn" style="width:740px; float:left;">
				<!--{if !empty($forumarchive)}-->
			...
			<!--{if !empty($_G['forum']['recommendlist'])}-->
				<div class="bm bmw xiaoyvu_recommen">
					<div class="bm_h cl">
