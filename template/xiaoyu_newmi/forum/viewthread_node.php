...
<!--{if $post['first']}-->
	<div id="xiaoyu_stamp"><img src="{if $_G['forum_threadstamp']}{STATICURL}image/stamp/$_G[forum_threadstamp][url]{else}{IMGDIR}/sigil.png{/if}" title="$_G[forum_threadstamp][text]" /></div>
<!--{/if}-->
  <h1> <span></span> <span></span> <span class="name">$_G[forum_thread][subject]</span> </h1> 
  <p class="txt authi">

<span>发表于 </span><span class="time">$post[dateline]</span>
...
<span>
<!--{if $post['invisible'] == 0}-->
<a href="forum.php?mod=viewthread&action=printable&tid=$_G[tid]" title="{lang thread_printable}" target="_blank"><img src="{IMGDIR}/print.png" alt="{lang thread_printable}" class="vm" /></a>
<!--{/if}-->
...
<!--{/if}-->
<span class="xg1">
</span><span class="f_r"><i class="msg"></i>$_G[forum_thread][allreplies]</span> <span class="f_r"><i class="see"></i>$_G[forum_thread][views]</span>
</p>
</div>
<!--{/if}-->

<!--{if empty($post['deleted'])}-->
<table id="pid$post[pid]" class="plhin {if !$post['first']}xiaoyu_view_reply{/if}" summary="pid$post[pid]" cellspacing="0" cellpadding="0" >
<tr>
	<!--{if !$postcount && !$_G['forum_thread']['archiveid'] && $post['first'] }--><td class="pls" rowspan="2" style="width:0px;"></td><!--{else}-->
	<td class="pls" rowspan="2">
		<div id="favatar$post[pid]" class="favatar">
			$post[newpostanchor] $post[lastpostanchor]
			<!--{if $post['authorid'] && $post['username'] && !$post['anonymous']}-->
				
				<div class="p_pop blk bui xm_card card_gender_$post['gender']" id="userinfo$post[pid]" style="display:none;">
					...
					<div class="c_info">
							<div class="c_info_top">
								...
								<a target="_blank" href="home.php?mod=spacecp&ac=usergroup&gid=$post[authorid]">{$post[authortitle]}</a>
							</div>
							{eval viewthread_profile_node('top', $post);}
					</div>
					
					<div class="c_action">                                          
                            <a onclick="showWindow('adduser', this.href, 'get', 0);" href="home.php?mod=spacecp&ac=friend&op=add&uid=$post[authorid]&handlekey=addfriendhk_$post[authorid]" class="y">加为好友</a>		
                            <a href="home.php?mod=spacecp&ac=pm&op=showmsg&handlekey=showmsg_$post[authorid]&touid=$post[authorid]&pmid=0&daterange=2&pid=$post[pid]&tid=$post[tid]" onclick="showWindow('sendpm', this.href);" title="{lang viewthread_left_sendpm}" class="y">{lang viewthread_left_sendpm}</a>
							
                            <a class="z" target="_blank" href="home.php?mod=space&uid=$post[authorid]">访问家园</a>
                        </div>
					</div>
	...
	<!--{if !$postcount && !$_G['forum_thread']['archiveid'] && $post['first'] }--><!--{else}-->
		<div class="pi"<!--{if $close_leftinfo && !$post['anonymous'] && $postshowavatars && $showavatars}--> style="height:48px"<!--{/if}-->>
				...
				<strong>
					<a href="{if $post[first]}forum.php?mod=viewthread&tid=$_G[tid]$fromuid{else}forum.php?mod=redirect&goto=findpost&ptid=$_G[tid]&pid=$post[pid]$fromuid{/if}"  {if $fromuid}title="{lang share_url_copy_comment}"{/if} id="postnum$post[pid]" onclick="setCopy(this.href, '{lang post_copied}');return false;">
						<!--{if isset($post[isstick])}-->
							<img src ="{IMGDIR}/settop.png" title="{lang replystick}" class="vm" /> 1{lang from} {$post[number]}{$postnostick}
			...
			<div class="pti">
				...
				<!--{else}-->
				<a href="home.php?mod=space&uid=$post[authorid]" target="_blank" class="auth_name">$post['author']</a>
				<!--{/if}-->
				...
				<div class="authi" style="display:none">
				<!--{if $post['authorid'] && !$post['anonymous']}-->
					<!--{if $_self}-->
					&nbsp;{lang thread_author}<span class="pipe">|</span>
					<!--{/if}-->
					<!--{if !$_G['setting']['authoronleft']}--><a href="home.php?mod=space&uid=$post[authorid]" target="_blank" class="xi2">$post[author]</a>$authorverifys<!--{/if}-->
					<!--{if $close_leftinfo}--><!--{/if}-->
					<span class="author"><a href="home.php?mod=space&uid=$post['authorid']" target="_blank" class="xw1"{if $post[groupcolor]} style="color: $post[groupcolor]"{/if}>$post[author]</a>$authorverifys</span>
					<em id="authorposton$post[pid]">&nbsp;<span>{lang poston} $post[dateline]</span></em>
					<!--{if $post['status'] & 8}-->
						<span class="xg1"><!--{if $_G['setting']['mobile']['mobilecomefrom']}-->{$_G['setting']['mobile']['mobilecomefrom']}<!--{else}-->{lang from_mobile}<!--{/if}--></span>
					<!--{/if}-->
					<!--{if $post['invisible'] == 0}-->
