...
<style> body{ background:#F1F2F6}</style>
<div id="ct" class="xiaoyu_post_art cl" style="width:960px; padding:19px; overflow:hidden"><h1 class="mbm bbs mt"><!--{if !empty($aid)}-->{lang article_edit}<!--{else}-->{lang article_publish}<!--{/if}--></h1>
	<div class="mn">
		<div class="bm bw0">
			...
			<form method="post" autocomplete="off" id="articleform" action="portal.php?mod=portalcp&ac=article{if $_GET[modarticlekey]}&modarticlekey=$_GET[modarticlekey]{/if}" enctype="multipart/form-data">
				...
				<div class="pbw">
					...
					<div style="border:1px solid #ddd; padding:3px; background:#EEE;border-radius:4px;height:400px;"><iframe src="home.php?mod=editor&charset={CHARSET}&allowhtml=1&isportal=1" name="uchome-ifrHtmlEditor" id="uchome-ifrHtmlEditor" scrolling="no" border="0" frameborder="0" style="width:100%;height:100%;position:relative;"></iframe></div>
				</div>
