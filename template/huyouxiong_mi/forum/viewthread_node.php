		...
		<div class="pi"<!--{if $close_leftinfo}--> style="{if !$comiis_view_tit_lz && $post['first']}display:none;{/if}<!--{if ($postcount==0 || $post['first'] || ($comiis_first == 1 && $postcount == 1)) && !$_GET['inajax']}-->border-top:none;<!--{/if}-->"<!--{/if}-->>
			...
			<div class="pti"<!--{if $post['first'] && $close_leftinfo}--> id="comiis_authi_author" style="display:none;"<!--{/if}-->>
				...
				<!--{if $post['authorid'] && !$post['anonymous']}-->
					<!--{if $comiis_view_zlb !=0 && $comiis_view_tit_avt==2 && $post['first']}-->
						<!--{if in_array($thread['displayorder'], array(1, 2, 3, 4))}--><span style="color:#ff4c4c;">{lang thread_stick}</span>&nbsp;&nbsp;<!--{/if}-->
						<!--{if $thread['digest'] > 0 && $filter != 'digest'}--><span style="color:#ff4c4c;">{lang thread_digest}</span>&nbsp;<!--{/if}-->
					<!--{/if}-->
					<!--{if $_self}-->
					...
					<a href="home.php?mod=space&uid=$post[authorid]" target="_blank" c="1" class="kmxi2">$post[author]</a> {$authorverifys}&nbsp;<a href="home.php?mod=spacecp&ac=usergroup&gid=$post[groupid]" target="_blank" class="vyhz">{$post[authortitle]}</a><!--{if !$post['first']}-->&nbsp;<!--{/if}-->
					<!--{/if}-->
					<em id="authorposton$post[pid]">
					<!--{if $post['first'] && ($_G['forum_thread']['typeid'] || $_G['forum_thread']['sortid'])}-->
						发表在&nbsp;
					<!--{else}-->
						{lang poston}
					<!--{/if}-->
					...
					<!--{/if}-->
					<!--{if $comiis_view_zlb !=0 && $comiis_view_tit_avt !=2}-->
						<!--{if in_array($thread['displayorder'], array(1, 2, 3, 4))}-->&nbsp;<span style="color:#ff4c4c;">{lang thread_stick}</span>&nbsp;&nbsp;<!--{/if}-->
						<!--{if $thread['digest'] > 0 && $filter != 'digest'}--><span style="color:#ff4c4c;">{lang thread_digest}</span>&nbsp;<!--{/if}-->
					<!--{/if}-->
					<!--{if $post['invisible'] == 0}-->
